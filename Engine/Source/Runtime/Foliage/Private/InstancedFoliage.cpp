	...
	: Super(ObjectInitializer)
{
	...
	bCastShadowForCSMAddShadowMask = true;
...
void FFoliageStaticMesh::UpdateComponentSettings(const UFoliageType_InstancedStaticMesh* InSettings)
{
	if (Component)
	{
		...
		//@StarLight code - Begin using Clustering Algorithm Based on Density to build tree, Added by zhuyule
		if (Component->InstanceMaxInterval != FoliageType->MaxInterval)
		{
			Component->InstanceMaxInterval = FoliageType->MaxInterval;
			bNeedsMarkRenderStateDirty = true;
		}
		if (Component->InstanceMinPts != FoliageType->MinPts)
		{
			Component->InstanceMinPts = FoliageType->MinPts;
			bNeedsMarkRenderStateDirty = true;
		}
		//@StarLight code - END   using Clustering Algorithm Based on Density to build tree, Added by zhuyule

		//@StarLight code - BEGIN GPU-Driven, Added by yanjianhong
		if (Component->bUseGpuDrivenForInstance != FoliageType->bUseGpuDrivenForInstance) {
			Component->bUseGpuDrivenForInstance = FoliageType->bUseGpuDrivenForInstance;
			bNeedsMarkRenderStateDirty = true; // Actually useless, you need to recreate Component to register to GpuDrivenSystem
		}
		...
		// @StarLight code - BEGIN MobileCSM+ShadowMask Created by yanjianhong
		if (Component->bCastShadowForCSMAddShadowMask != FoliageType->bCastShadowForCSMAddShadowMask) {
			Component->bCastShadowForCSMAddShadowMask = FoliageType->bCastShadowForCSMAddShadowMask;
			bNeedsMarkRenderStateDirty = true;
		}
		// @StarLight code - END MobileCSM+ShadowMask Created by yanjianhong
