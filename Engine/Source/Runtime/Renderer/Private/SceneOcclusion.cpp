...
DECLARE_CYCLE_STAT(TEXT("HZBOcclusion Test"), STAT_CLMM_HZBOcclusionTest, STATGROUP_CommandListMarkers);
...
{
#if SL_USE_MOBILEHZB
	MobileHzbPrimitives.Reserve(MobileSizeX * MobileSizeY);
...
{
#if SL_USE_MOBILEHZB
	uint32 Index = MobileHzbPrimitives.AddUninitialized();
	check(Index < MobileSizeX* MobileSizeY);
	MobileHzbPrimitives[Index].Center = BoundsCenter;
	MobileHzbPrimitives[Index].Extent = BoundsExtent;
	return Index;
#else
...
#if SL_USE_MOBILEHZB
	checkSlow(Index < MobileSizeX * MobileSizeY);
...
public:
		...
		LAYOUT_FIELD(FShaderResourceParameter, BoundsExtentTexture)
		LAYOUT_FIELD(FShaderResourceParameter, BoundsExtentSampler)
		LAYOUT_FIELD(FShaderParameter, SLPreViewTranslation)
		LAYOUT_FIELD(FShaderParameter, SLTranslatedWorldToClip)
		...
		: FGlobalShader(Initializer)
	{
		...
		BoundsExtentTexture.Bind(Initializer.ParameterMap, TEXT("BoundsExtentTexture"));
		BoundsExtentSampler.Bind(Initializer.ParameterMap, TEXT("BoundsExtentSampler"));
		SLPreViewTranslation.Bind(Initializer.ParameterMap, TEXT("SLPreViewTranslation"));
		SLTranslatedWorldToClip.Bind(Initializer.ParameterMap, TEXT("SLTranslatedWorldToClip"));
	}
	...
	void SetParameters(FRHICommandList& RHICmdList, const FViewInfo& View, FRHITexture* BoundsCenter, FRHITexture* BoundsExtent, FRHITexture* MobileHzbTexture)
	{
		...
		//FGlobalShader::SetParameters<FViewUniformShaderParameters>(RHICmdList, ShaderRHI, View.ViewUniformBuffer);
		//GLSL不对应Sampler总是会出错,如果不是Vulkan还是绑定对应数量的Sampler
		...
		SetTextureParameter(RHICmdList, ShaderRHI, BoundsCenterTexture, BoundsCenterSampler, TStaticSamplerState<SF_Point, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI(), BoundsCenter);
		SetTextureParameter(RHICmdList, ShaderRHI, BoundsExtentTexture, BoundsExtentSampler, TStaticSamplerState<SF_Point, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI(), BoundsExtent);
		SetShaderValue(RHICmdList, ShaderRHI, SLPreViewTranslation, View.ViewMatrices.GetPreViewTranslation());
		SetShaderValue(RHICmdList, ShaderRHI, SLTranslatedWorldToClip, View.ViewMatrices.GetTranslatedViewProjectionMatrix());
	}
};

IMPLEMENT_SHADER_TYPE(, FMobileHZBTestPS, TEXT("/Engine/Private/MobileHZBOcclusion.usf"), TEXT("HZBTestPS"), SF_Pixel);
...
void FHZBOcclusionTester::MobileSubmit(FRHICommandListImmediate& RHICmdList, const FViewInfo& View)
{
	...
	// Draw test
	{
		...
		RHICmdList.TransitionResource(EResourceTransitionAccess::EReadable, EResourceTransitionPipeline::EGfxToGfx, MobileHZBTexture);
		...
		RHICmdList.BeginRenderPass(RPInfo, TEXT("TestHZB"));
		{
			...
			DrawRectangle(
				RHICmdList,
				0, 0,
				MobileSizeX, MobileSizeY,
				0, 0,
				MobileSizeX, MobileSizeY,
				FIntPoint(MobileSizeX, MobileSizeY),
				FIntPoint(MobileSizeX, MobileSizeY),
				VertexShader,
				EDRF_UseTriangleOptimization);
		}
...
void FHZBOcclusionTester::WriteOccludedData(FRHICommandListImmediate& RHICmdList, uint32 CurFrameIndex) {
	...
	const uint32 NumPrimitives = MobileHzbPrimitives.Num();
	for (uint32 i = 0; i < NumPrimitives; i++)
	{
		const FOcclusionPrimitive& Primitive = MobileHzbPrimitives[i];
