...
static TAutoConsoleVariable<int32> CVarMaxInterval(
	TEXT("foliage.MaxInterval"),
	400,
	TEXT("The Max interval can be generated links"));

static TAutoConsoleVariable<int32> CVarMinPts(
	TEXT("foliage.MinPts"),
	4,
	TEXT("The Min Linked Num for the center point "));

static TAutoConsoleVariable<int32> CVarUseCulsterForFoliage(
	TEXT("foliage.UseCulster"),
	0,
	TEXT("If greater than zero, use the culster analysis to build tree for the foliage"));
...
protected:
	...
	struct FLinkPair
	{
		int32 Num;
		int32 Index;
		TArray<int32> LinkIndexs;

		FLinkPair(int32 InIndex)
			: Index(InIndex)
		{
			Num = 0;
		}

		void AddLinkIndex(int32 InIndex)
		{
			LinkIndexs.AddUnique(InIndex);
			Num = LinkIndexs.Num();
		}

		bool operator==(const FLinkPair& Other) const { return Index == Other.Index; }

		bool operator== (int32 InIndex) const { return Index == InIndex; }
		
		bool operator< (const FLinkPair& Other) const
		{
			return Num > Other.Num;
		}
	};
	TArray<FLinkPair> LinkPairs;

	TArray<int32> LinkPairIndexs;
	...
	struct FDensityPair
	{
		...
		TArray<int32> Indexs;
		TArray<int32> ClusterCenterIndexs;

		FDensityPair(int32 InStart)
			: Start(InStart)
		{
			Indexs.Add(InStart);
			Num = 1;
		...
		{
			Indexs.AddUnique(Index);
			Num = Indexs.Num();
		}

		void AddCenterIndex(int32 Index)
		{
			ClusterCenterIndexs.AddUnique(Index);
		}
	};
	TArray<FDensityPair> DensityPairs;

	void NewSplit(int32 InNum)
	{
		checkSlow(InNum);
		Clusters.Reset();
		...
		OriginalIndex.Append(SortIndex);

		int32 MaxInterval = ClusterMaxInterval > 0 ? ClusterMaxInterval : CVarMaxInterval.GetValueOnAnyThread();
		for (int32 i = 0; i < OriginalIndex.Num(); i++)
		{
			LinkPairs.Add(FLinkPair(i));
			FLinkPair& Node = LinkPairs.Last();
			for (int32 j = 0; j < OriginalIndex.Num(); j++)
			{
				if (i != j && FVector::Dist(Transforms[j].GetOrigin(), Transforms[i].GetOrigin()) < MaxInterval)
				{
					Node.AddLinkIndex(j);
				}
			}
		...
		int32 TotalNum = 0;
		for (int32 Index = 0; Index < DensityPairs.Num(); Index++)
		{
			DensityPairs[Index].Indexs.Sort();
			Clusters.Add(FRunPair(TotalNum, DensityPairs[Index].Indexs.Num()));

			SortIndex.Append(DensityPairs[Index].Indexs);
			TotalNum += DensityPairs[Index].Indexs.Num();
		}

		checkSlow(InNum == SortIndex.Num());
		checkSlow(Clusters.Num() > 0);
		int32 At = 0;
		for (auto& Cluster : Clusters)
		{
	...
	void NewSplit()
	{
		...
		LinkPairIndexs.Reset();
		for (int32 Index = 0; Index < LinkPairs.Num(); Index++)
		{
			LinkPairIndexs.Add(LinkPairs[Index].Index);
		}

		int32 MinPts = ClusterMinPts > 0 ? ClusterMinPts : CVarMinPts.GetValueOnAnyThread();
		while (LinkPairs.Num() >= 1)
		{
			FLinkPair LinkNode = LinkPairs[0];

			DensityPairs.Add(FDensityPair(LinkNode.Index));
			FDensityPair& Node = DensityPairs.Last();

			bool bMaxLeaf = false;

			for (int32 Index = 0; Index < LinkNode.LinkIndexs.Num(); Index++)
			{
				Node.AddItem(LinkNode.LinkIndexs[Index]);
				if (Node.Num >= BranchingFactor)
				{
					bMaxLeaf = true;
					break;
				}
			...
			if (!bMaxLeaf && LinkNode.LinkIndexs.Num() >= MinPts)
			{
				Node.ClusterCenterIndexs.Append(LinkNode.LinkIndexs);

				Node.ClusterCenterIndexs.AddUnique(LinkNode.Index);

				TArray<int32> CenterIndexs;
				CenterIndexs.Append(Node.ClusterCenterIndexs);
				while (!bMaxLeaf && CenterIndexs.Num())
				{
					int32 Index = CenterIndexs[0];
					CenterIndexs.RemoveAt(0);

					int32 LinkIndexTemp = LinkPairIndexs.Find(Index);

					if (LinkIndexTemp != INDEX_NONE)
					{
						FLinkPair LinkTemp = LinkPairs[LinkIndexTemp];

						if (LinkTemp.LinkIndexs.Num() >= MinPts)
						{
							for (int32 i = 0; i < LinkTemp.LinkIndexs.Num(); i++)
							{
								int32 IndexTemp = LinkTemp.LinkIndexs[i];
								if (Node.ClusterCenterIndexs.Find(IndexTemp) == INDEX_NONE)
								{
									CenterIndexs.AddUnique(IndexTemp);
									Node.ClusterCenterIndexs.AddUnique(IndexTemp);
								}
							}

							for (int32 i = 0; i < LinkTemp.LinkIndexs.Num(); i++)
							{
								Node.AddItem(LinkTemp.LinkIndexs[i]);
								if (Node.Num >= BranchingFactor)
								{
									bMaxLeaf = true;
									break;
			...
			for (int32 Index = 0; Index < Node.Indexs.Num(); )
			{
				int32 LinkIndexTemp = LinkPairIndexs.Find(Node.Indexs[Index]);
				if (LinkIndexTemp != INDEX_NONE)
				{
					LinkPairIndexs.RemoveAt(LinkIndexTemp);
					LinkPairs.RemoveAt(LinkIndexTemp);
					Index++;
				}
				else
				{
					Node.Indexs.RemoveAt(Index);
				}
			}
		}
	}
...
public:
	...
	FClusterBuilder(TArray<FMatrix> InTransforms, TArray<float> InCustomDataFloats, int32 InNumCustomDataFloats, const FBox& InInstBox, int32 InMaxInstancesPerLeaf, float InDensityScaling, int32 InInstancingRandomSeed, int32 InMaxInterval, int32 InMinPtrs, bool InGenerateInstanceScalingRange)
		...
		, ClusterMaxInterval(InMaxInterval)
		, ClusterMinPts(InMinPtrs)
	...
	void BuildTree()
	{
		...
		//@StarLight code - BEGIN using Clustering Algorithm Based on Density to build tree, Added by zhuyule
		int32 bUseCulsterForFoliage = CVarUseCulsterForFoliage.GetValueOnAnyThread();
		if (bUseCulsterForFoliage > 0)
		{
			NewSplit(Num);
		}
		//@StarLight code - END   using Clustering Algorithm Based on Density to build tree, Added by zhuyule
...
static void TestFoliage(const TArray<FString>& Args)
{
	...
	FClusterBuilder Builder(InstanceTransforms, InstanceCustomDataDummy, 0, TempBox, 16, 1.0f, 1, 0, 0, 0);
	Builder.BuildTree();
...
void UHierarchicalInstancedStaticMeshComponent::BuildTree()
{
	...
	if (bMeshIsValid)
	{
		...
		FClusterBuilder Builder(InstanceTransforms, PerInstanceSMCustomData, NumCustomDataFloats, GetStaticMesh()->GetBounds().GetBox(), DesiredInstancesPerLeaf(), CurrentDensityScaling, InstancingRandomSeed, InstanceMaxInterval, InstanceMinPts, PerInstanceSMData.Num() > 0);
	...
	)
{
	...
	FClusterBuilder Builder(InstanceTransforms, InstanceCustomDataFloats, NumCustomDataFloats, MeshBox, MaxInstancesPerLeaf, DensityScaling, InstancingRandomSeed, InMaxInterval, InMinPtr, InGenerateInstanceScalingRange);
	Builder.BuildTree();
...
void UHierarchicalInstancedStaticMeshComponent::BuildTreeAsync()
{
	...
	if (bMeshIsValid)
	{
		...
		TSharedRef<FClusterBuilder, ESPMode::ThreadSafe> Builder(new FClusterBuilder(InstanceTransforms, PerInstanceSMCustomData, NumCustomDataFloats, GetStaticMesh()->GetBounds().GetBox(), DesiredInstancesPerLeaf(), CurrentDensityScaling, InstancingRandomSeed, InstanceMaxInterval, InstanceMinPts, PerInstanceSMData.Num() > 0));
